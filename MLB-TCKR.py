...
def get_settings():
    defaults = {
        ...
        "telemetry_enabled": True,   # Usage telemetry (PostHog) enabled by default (opt-out)
        "telemetry_debug_logging": True,  # Print telemetry send diagnostics to console
        "telemetry_install_id": "", # Anonymous persistent install UUID (distinct_id)
        "telemetry_posthog_key": "", # PostHog project API key (or set POSTHOG_PROJECT_API_KEY env var)
    ...
    # Ensure each install has a stable anonymous distinct_id for telemetry.
    if not defaults.get('telemetry_install_id'):
        defaults['telemetry_install_id'] = str(uuid.uuid4())
        try:
...
class TelemetryClient:
    """Minimal non-blocking PostHog capture client.

    Sends anonymous usage events without blocking UI/render loops.
    """

    def __init__(self, settings):
        self.enabled = bool(settings.get('telemetry_enabled', True))
        self.debug = bool(settings.get('telemetry_debug_logging', True))
        self.distinct_id = str(settings.get('telemetry_install_id', '')).strip()
        _cfg_key = str(settings.get('telemetry_posthog_key', '')).strip()
        _env_key = str(os.environ.get('POSTHOG_PROJECT_API_KEY', '')).strip()
        self.api_key = _cfg_key or _env_key
        self._key_source = 'settings' if _cfg_key else ('env' if _env_key else 'none')
        self.host = str(settings.get('telemetry_posthog_host', 'https://us.i.posthog.com')).rstrip('/')
        self._warned_no_key = False
        if self.debug:
            _id_short = self.distinct_id[:8] if self.distinct_id else 'none'
            print(
                f"[TELEMETRY] init enabled={self.enabled} key_source={self._key_source} "
                f"host={self.host} distinct_id={_id_short}"
            )

    def capture(self, event, properties=None):
        if not self.enabled:
            if self.debug:
                print(f"[TELEMETRY] skipped event={event} reason=disabled")
            return
        if not self.api_key:
            if not self._warned_no_key:
                print("[TELEMETRY] telemetry enabled but no PostHog API key configured; skipping sends")
                self._warned_no_key = True
            return
        if not self.distinct_id:
            if self.debug:
                print(f"[TELEMETRY] skipped event={event} reason=missing_distinct_id")
            return

        props = dict(properties or {})
        props.setdefault('app_version', VERSION)
        props.setdefault('platform', platform.platform())
        props.setdefault('python_version', platform.python_version())
        props.setdefault('frozen', bool(getattr(sys, 'frozen', False)))
        props['distinct_id'] = self.distinct_id
        ...
        if self.debug:
            print(f"[TELEMETRY] queue event={event}")

        def _send():
            try:
                _resp = requests.post(f"{self.host}/capture/", json=payload, timeout=2)
                if self.debug:
                    if 200 <= _resp.status_code < 300:
                        print(f"[TELEMETRY] sent event={event} status={_resp.status_code}")
                    else:
                        _preview = (_resp.text or '').strip().replace('\n', ' ')[:200]
                        print(f"[TELEMETRY] send_failed event={event} status={_resp.status_code} body={_preview}")
            except Exception as _te:
                if self.debug:
                    print(f"[TELEMETRY] send_error event={event} err={_te}")

        QtCore.QThreadPool.globalInstance().start(_TelemetrySendRunnable(_send))


class _TelemetrySendRunnable(QtCore.QRunnable):
    """Run a Python callable on Qt's global thread pool."""
...
def get_telemetry_client(settings=None):
    """Return singleton telemetry client, creating it lazily from settings."""
    global _TELEMETRY_CLIENT
    if _TELEMETRY_CLIENT is None:
        _TELEMETRY_CLIENT = TelemetryClient(settings or get_settings())
    return _TELEMETRY_CLIENT


def reset_telemetry_client(settings=None):
    """Reset singleton telemetry client so new settings take effect immediately."""
    global _TELEMETRY_CLIENT
    _TELEMETRY_CLIENT = TelemetryClient(settings or get_settings())
    return _TELEMETRY_CLIENT
...
class MLBTickerWindow(QtWidgets.QWidget):
    def __init__(self):
        ...
        self._telemetry = get_telemetry_client(self.settings)
        self._run_id = str(uuid.uuid4())
        self._run_started_monotonic = time.monotonic()
        self._telemetry_run_ended = False
        self.games = []
        ...
        self._fetch_started_monotonic = 0.0
        self._fetch_stall_timeout_s = float(self.settings.get('fetch_stall_timeout_seconds', 90))
        ...
        # Telemetry context used by app_run_started/app_run_ended and settings events.
        self._telemetry_context = {
            'monitor_index': int(_mon_idx),
            'ticker_height': int(self.ticker_height),
            'screen_width': int(screen.width()),
            'screen_height': int(screen.height()),
            'screen_dpr': float(self.dpr),
        }
        ...
        # Fetch watchdog — recovers from stuck background fetches (for example,
        # network calls hanging after lock/sleep resume) so polling can continue.
        self._fetch_watchdog = QtCore.QTimer(self)
        self._fetch_watchdog.setInterval(5000)
        self._fetch_watchdog.timeout.connect(self._check_fetch_stall)
    ...
    def _emit_run_start(self):
        props = dict(self._telemetry_context)
        props.update({
            'run_id': self._run_id,
            'started_at_unix': int(time.time()),
        })
        self._telemetry.capture('app_run_started', props)

    def _emit_run_end(self, end_reason):
        if self._telemetry_run_ended:
            return
        self._telemetry_run_ended = True
        props = dict(self._telemetry_context)
        props.update({
            'run_id': self._run_id,
            'end_reason': end_reason,
            'duration_sec_rough': int(max(0.0, time.monotonic() - self._run_started_monotonic)),
            'ended_at_unix': int(time.time()),
        })
        self._telemetry.capture('app_run_ended', props)
    ...
    def setup_appbar(self):
        ...
        # Step 8: Broadcast WM_SETTINGCHANGE to ALL top-level windows.
        #
        # ABM_SETPOS updates the Shell's internal work-area record, and
        # ABM_WINDOWPOSCHANGED tells the Shell our bar has moved — but
        # neither automatically notifies every process that the work area
        # has changed.  The Window Manager caches the work area for
        # CW_USEDEFAULT placement, so without this broadcast, new windows
        # will still use the stale (full-screen) work area and open
        # underneath the ticker bar.  When the user drags such a window,
        # Windows Snap re-reads the *current* (correct) work area and
        # snaps it below the bar — which is why it only repositions after
        # being moved.  Broadcasting WM_SETTINGCHANGE forces all processes
        # (including Explorer's window-placement code) to invalidate their
        # cached work area immediately, matching what the Windows Taskbar
        ...
        # Telemetry: capture the exact AppBar registration geometry.
        if hasattr(self, '_telemetry') and self._telemetry is not None:
            self._telemetry.capture('appbar_registered', {
                'run_id': self._run_id,
                'dpr': float(dpr),
                'monitor_phys_left': int(phys_x),
                'monitor_phys_top': int(phys_y),
                'monitor_phys_right': int(phys_x + phys_width),
                'monitor_phys_bottom': int(phys_y + phys_height),
                'reserved_phys_left': int(abd.rc.left),
                'reserved_phys_top': int(abd.rc.top),
                'reserved_phys_right': int(abd.rc.right),
                'reserved_phys_bottom': int(abd.rc.bottom),
                'logical_height_px': _logical_height,
            })
    ...
    def remove_appbar(self):
        ...
        try:
            ...
            # Broadcast WM_SETTINGCHANGE so all windows know the reserved
    ...
    def _is_fetch_stale(self):
        """Return True when the current fetch lock has exceeded the stall timeout."""
        if not self.is_fetching:
            return False
        if self._fetch_started_monotonic <= 0:
            return True
        return (time.monotonic() - self._fetch_started_monotonic) > self._fetch_stall_timeout_s

    def _detach_data_worker(self, worker):
        """Disconnect worker signals from this window so stale workers can't mutate UI state."""
        if not worker:
            return
        _pairs = (
            (worker.data_fetched, self.on_data_received),
            (worker.fetch_error, self.on_fetch_error),
            (worker.finished, self.on_fetch_complete),
            (worker.teams_known, self._on_teams_known),
            (worker.data_fetched, self.on_preview_data_received),
            (worker.fetch_error, self._on_preview_fetch_error),
        )
        for _sig, _slot in _pairs:
            try:
    ...
    def _force_unlock_fetch(self, reason):
        """Clear fetch lock and detach current worker after a detected stall."""
        _elapsed = max(0.0, time.monotonic() - self._fetch_started_monotonic)
        print(f"[MLB] Forcing fetch unlock ({reason}) after {_elapsed:.1f}s")
        _stale_worker = self.data_worker
        self.is_fetching = False
        self._fetch_started_monotonic = 0.0
        self._loading_mode = False
        if _stale_worker:
            self._detach_data_worker(_stale_worker)
            if self.data_worker is _stale_worker:
                self.data_worker = None
        self.update()

    def _check_fetch_stall(self):
        """Watchdog: auto-recover if a data fetch thread appears stuck."""
        if not self._is_fetch_stale():
            return
        _elapsed = max(0.0, time.monotonic() - self._fetch_started_monotonic)
        print(f"[MLB] Fetch stall detected ({_elapsed:.1f}s) — recovering")
        self._force_unlock_fetch("watchdog timeout")
        self.start_data_fetch(show_loading=False)
    ...
    def start_data_fetch(self, show_loading=None):
        ...
        if self.is_fetching:
            if self._is_fetch_stale():
                _elapsed = max(0.0, time.monotonic() - self._fetch_started_monotonic)
                print(f"[MLB] Stale fetch lock detected ({_elapsed:.1f}s) — recovering")
                self._force_unlock_fetch("stale fetch lock")
            else:
    ...
    def start_preview_fetch(self):
        ...
        if self.is_fetching:
            if self._is_fetch_stale():
                _elapsed = max(0.0, time.monotonic() - self._fetch_started_monotonic)
                print(f"[MLB] Stale preview fetch lock detected ({_elapsed:.1f}s) — recovering")
                self._force_unlock_fetch("stale preview lock")
            else:
                return
        self.is_fetching = True
        self._fetch_started_monotonic = time.monotonic()
    ...
    def build_game_pixmap(self, game, force_show_player_info=False):
        ...
        # symmetry must be based on what is actually rendered, not reserved widths.
        _away_outer_slack = 0
        _home_outer_slack = 0
        ...
        # Measure true visual extents via tightBoundingRect.  This is independent
        # of typographic ascent/descent values, which vary wildly between fonts and
        # OS scaling settings.  Team names are drawn ALL-CAPS, so row-0 uses an
        # uppercase-only sample (no descenders, giving equal top/bottom whitespace).
        _m_tbr = metrics.tightBoundingRect('ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789')
        _m_top = -_m_tbr.top()            # pixels above baseline (visual ascent)
        _m_bot = max(0, _m_tbr.bottom())  # pixels below baseline (visual descent)
        _m_h   = _m_top + _m_bot          # total visual row-0 height

        _s_tbr = small_metrics.tightBoundingRect(
            'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789.,: -%')
        _s_top = -_s_tbr.top()
        _s_bot = max(0, _s_tbr.bottom())
        _s_h   = _s_top + _s_bot

        _t_tbr = tiny_metrics.tightBoundingRect(
            'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789.,: -%')
        _t_top = -_t_tbr.top()
        _t_bot = max(0, _t_tbr.bottom())
        _t_h   = _t_top + _t_bot

        _gap1 = 3  # px between row-0 baseline and top of row-1 glyphs
        _gap2 = 2  # px between row-1 baseline and top of row-2 glyphs
        ...
        if _want_row1 and _want_row2:
            if _m_h + _gap1 + _s_h + _gap2 + _t_h > _content_h:
                _want_row2 = False  # drop pitcher/batter line first
        if _want_row1:
            if _m_h + _gap1 + _s_h > _content_h:
                _want_row1 = False  # drop record/subtext line if still too tall

        # Single centering calculation — all baselines derived from one block_top.
        if _want_row1 and _want_row2:
            _total_h = _m_h + _gap1 + _s_h + _gap2 + _t_h
        elif _want_row1:
            _total_h = _m_h + _gap1 + _s_h
        else:
            _total_h = _m_h

        _block_top = (_content_h - _total_h) // 2
        text_y   = _block_top + _m_top
        record_y = (_block_top + _m_h + _gap1 + _s_top) if _want_row1 else None
        detail_y = (_block_top + _m_h + _gap1 + _s_h + _gap2 + _t_top) if _want_row2 else None

        # Use effective (actually drawn) lower-row widths for WP-bar symmetry.
        _away_row_effective_w = 0
        _home_row_effective_w = 0
        if _want_row1:
            _away_row1_text = away_detail_text if show_records else (away_record_text if show_wl else '')
            _home_row1_text = home_detail_text if show_records else (home_record_text if show_wl else '')
            if _away_row1_text:
                _away_row_effective_w = small_metrics.horizontalAdvance(_away_row1_text)
            if _home_row1_text:
                _home_row_effective_w = small_metrics.horizontalAdvance(_home_row1_text)
        if _want_row2 and show_records:
            if away_live_detail:
                _away_row_effective_w = max(_away_row_effective_w, tiny_metrics.horizontalAdvance(away_live_detail))
            if home_live_detail:
                _home_row_effective_w = max(_home_row_effective_w, tiny_metrics.horizontalAdvance(home_live_detail))

        _away_outer_content_w = max(away_top_w, _away_row_effective_w)
        _home_outer_content_w = max(home_top_w, _home_row_effective_w)
        _away_outer_slack = max(0, away_block_width - _away_outer_content_w)
        _home_outer_slack = max(0, home_block_width - _home_outer_content_w)

        # All other vertically-centered elements share the same _content_h center.
        logo_y = (_content_h - logo_size) // 2
        time_y = text_y

        # odds_y: align small-font cap-top with main-font cap-top.
...
class SettingsDialog(QtWidgets.QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.ensurePolished()
        _content_w = self._general_container.sizeHint().width()
    ...
    def create_admin_tab(self):
        ...
        self.telemetry_posthog_key_edit.setPlaceholderText("Enter PostHog Project API key…")
        self.telemetry_posthog_key_edit.setText(self.settings.get('telemetry_posthog_key', ''))
        self.telemetry_posthog_key_edit.setToolTip(
            "PostHog Project API key used by this app for telemetry capture."
        )
    ...
    def _collect_settings(self):
        ...
        # Apply telemetry config changes immediately for subsequent events.
        reset_telemetry_client(self.settings)
        _parent = self.parent()
        if _parent is not None and hasattr(_parent, '_telemetry'):
            _parent._telemetry = get_telemetry_client(self.settings)

        # Telemetry: settings save event (counts changed keys, no sensitive values).
        changed_keys = sorted([k for k in self.settings.keys() if old_settings.get(k) != self.settings.get(k)])
        _screens = QtWidgets.QApplication.screens()
        _mon_idx = min(int(self.settings.get('monitor_index', 0)), max(0, len(_screens) - 1))
        _screen = _screens[_mon_idx] if _screens else QtWidgets.QApplication.primaryScreen()
        _geo = _screen.geometry() if _screen else QtCore.QRect(0, 0, 0, 0)
        _dpr = _screen.devicePixelRatio() if _screen else 1.0
        get_telemetry_client(self.settings).capture('settings_saved', {
            'changed_keys_count': len(changed_keys),
            'changed_keys': changed_keys,
            'monitor_index': _mon_idx,
            'ticker_height': int(self.settings.get('ticker_height', 64)),
            'screen_width': int(_geo.width()),
            'screen_height': int(_geo.height()),
...
def main():
    ...
    app.aboutToQuit.connect(lambda: window._emit_run_end('about_to_quit'))
